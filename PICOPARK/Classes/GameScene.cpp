...
bool GameScene::init()
{
    ...
    auto kill = Sprite::create("Kill.png");
    kill->setPosition(197, 50);
    kill->setScale(1.1);
    MyBodyParser::getInstance()->parseJsonFile("Kill.json");
    auto killBody = MyBodyParser::getInstance()->bodyFormJson(kill, "Kill.png", PhysicsMaterial(0.5, 0.5, 0.5));
    if (killBody != nullptr) {

        killBody->setDynamic(false);
        kill->setPhysicsBody(killBody);
    }
    this->addChild(kill, 1);


    auto ard1 = Sprite::create("120.png");
    ard1->setPosition(400, 0);
    ard1->setScale(0.5);
    MyBodyParser::getInstance()->parseJsonFile("nn.json");
    auto ardBody1 = MyBodyParser::getInstance()->bodyFormJson(ard1, "120.png", PhysicsMaterial(0.2, 0.2, 0.2));
    if (ardBody1 != nullptr) {

        ardBody1->setDynamic(false);
        ard1->setPhysicsBody(ardBody1);
    }
    this->addChild(ard1, 1);

    auto bab = Sprite::create("33.png");
    bab->setScale(1.2);
    bab->setPosition(350, 104);
    ...
    eventListner->onKeyPressed = [](EventKeyboard::KeyCode KeyCode, Event* event) {
        ...
        case EventKeyboard::KeyCode::KEY_W:
            event->getCurrentTarget()->runAction(JumpBy::create(0.5, Vec2(30, 0), 50, 1));
            break;
    ...
    auto doorbtn = MenuItemImage::create("33.png", "b.png", CC_CALLBACK_1(GameScene::GoToLevel3Scene, this));
    doorbtn->setScale(0.5);
    doorbtn->setPosition(444, 105);
...
void GameScene::GoToLevel3Scene(cocos2d::Ref* sender)
{
